...
void Vector4::SetColor(unsigned char r, unsigned char g, unsigned char b)
{
    color[0] = r;
    color[1] = g;
    color[2] = b;
...
double Vector4::GetRed()
{
    return color[0];
}

double Vector4::GetGreen()
{
    return color[1];
}

double Vector4::GetBlue()
{
    return color[2];
}
...
Vector4 operator* (const Matrix4 gMatrix,Vector4 gVector)
{
 ...
 tmp.SetColor(gVector.GetRed(), gVector.GetGreen(), gVector.GetBlue());
 return tmp;
}
